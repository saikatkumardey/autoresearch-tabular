"""
prepare.py — Diabetes regression (sklearn built-in dataset).
...
To use: copy this file to the repo root, replacing the default prepare.py.
  cp examples/diabetes/prepare.py . && uv run prepare.py && uv run train.py
...
def load_and_preprocess():
    data = load_diabetes()
    X = data.data.astype(np.float32)   # already normalized by sklearn
